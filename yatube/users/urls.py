from django.contrib.auth.views import LogoutView, PasswordResetView, \
    PasswordResetDoneView, LoginView, PasswordChangeView, \
...
urlpatterns = [
    ...
    path('password_reset/',
         PasswordResetView.as_view(template_name='users/password_reset_form.'
                                                 'html',
                                   success_url=reverse_lazy('users:password_'
                                                            'reset_done')),
         name='password_reset_form'),
    path('password_reset/done/',
         PasswordResetDoneView.as_view(template_name='users/password_reset_'
                                                     'done.html'),
         name='password_reset_done'),
    path('reset/<slug:uidb64>/<slug:token>/', PasswordResetConfirmView.
